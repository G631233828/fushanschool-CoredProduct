...
<body>
                             ...
                             <table class="table table-striped table-bordered table-hover dataTables-example">
                               ...
                               <c:forEach items="${pageList}" var="item" varStatus="status">
                                    <tr class="gradeX">
                                        ...
                                        <td>
                                        <c:choose>
                                        	<c:when test="${item.type=='JNOTIF'}">
                                        		全部通知
                                        	</c:when>
                                        	<c:when test="${item.type=='SNOTIF'}">
                                        		内部通知
                                        	</c:when>
                                        	<c:when test="${item.type=='ANOTIF'}">
                                        		家校通知
                                        	</c:when>
                                        </c:choose>
                                        ...
                                        <td class="center">
                                         	<button type="button" class="btn btn-primary btn-xs edit-news" data-id="1"
                                         	 onclick="updateFormValue('${item.id}','${item.title}','${item.author}',
                                         	 '${item.sendDate}','${item.type}','${item.content}')">编辑</button>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                 ...
                 <form id="add-news-form" action="${pageContext.request.contextPath}/school/notifiction/createOrUpdateToFind" 
                ...
                <div class="modal-body">
                      <div class="row">
                      	<div class="col-lg-6">
	                      	 <div class="form-group">
	                            ...
	                            <div class="col-sm-10">
	                                ...
	                                id="title" name="title" class="form-control" placeholder="标题" >
	                            </div>
	                        </div>
	                        <div class="form-group">
	                            ...
	                            <select class="form-control" name="type" id="type">
	                            	<option value="JNOTIF">全部通知</option>
	                            	<option value="SNOTIF">内部通知</option>
	                            	<option value="ANOTIF">家校通知</option>
	                            </select>
	                            </div>
	                        </div>
                      	</div>
                      	<div class="col-lg-6">
                      		 <div class="form-group">
	                            ...
	                            <div class="col-sm-10">
	                                <input required="" aria-required="true" type="text" 
	                                id="author" name="author" class="form-control" placeholder="作者" >
	                            </div>
                        	</div>
							<div class="form-group">
	                            <label class="col-sm-2 control-label">日期</label>
	                            <div class="col-sm-10">
	                            	<input type="text" value="" id="sendDate" name="sendDate" class="form-control">
	                            </div>
	                        </div>
                      ...
                      <div class="col-lg-12">
                            ...
                            <div class="col-sm-11">
                            	<textarea id="content" name="content" data-rangelength="[2,1000]" placeholder="备注" class="form-control parsley-validated" rows="6"></textarea>
                            </div>
                        </div>
                        </div>
                      </div>
 ...
 <script type="text/javascript">
 	//更新内容传值
 	function updateFormValue(id,title,author,sendDate,type,content){
 		$('#myModal5').modal('show');
 		$("#id").val(id);
		$("#title").val(title);
		$("#author").val(author);
