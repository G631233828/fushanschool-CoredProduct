...
<!--                                          teacherUser, studentUser, otheruser, SchoolManager, superManager -->
 										...
 										<c:choose>
                                        	<c:when test="${item.type=='superManager'}">
                                        			超级管理员
                                        	</c:when>
                                        	<c:when test="${item.type== 'teacherUser'}">
                                        			学校帐号
                                        	</c:when>
                                        	<c:otherwise>
                                        			网站注册帐号
                      ...
                      <div class="row">
                      	<div class="col-lg-6">
                        	...
                        	<div class="form-group">
	                            ...
	                            <div class="col-sm-10">
	                                <input type="text" id="cof_password" name="cof_password" class="form-control" 
	                                placeholder="确认密码" >
	                            </div>
                        	</div>
...
<!-- 	                                class="form-control" placeholder="选择性别" > -->
					...
					<input id="schoolid" name="schoolid" type="hidden" value="${sessionScope.loginUser.schoolId }">
                <div class="modal-footer">
                    <button type="button" class="btn btn-white" data-dismiss="modal">关闭</button>
 ...
 <script type="text/javascript">
 	//更新内容传值
 	function updateFormValue(id,username,password,type,weChatCode,headImage,remark,name,card,role){
 		$('#myModal5').modal('show');
 		$("#id").val(id);
