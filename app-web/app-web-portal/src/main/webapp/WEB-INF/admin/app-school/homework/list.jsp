...
<body>
                                    ...
                                    <tr class="gradeX">
                                        ...
                                        <td class="center">
                                         	<button type="button" class="btn btn-primary btn-xs edit-news" data-id="1"
                                         	 onclick="updateFormValue('${item.id}','${item.name}','${item.sdate}','${item.dempartment}',
                                         	 '${item.edate}','${item.type}','${item.message}','${item.repairsAddredd}')">编辑</button>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                      <div class="row">
                      	<div class="col-lg-6">
	                      	 <div class="form-group">
	                            <label class="col-sm-2 control-label">审请人</label>
	                            <div class="col-sm-10">
	                                <input required="" aria-required="true" type="text" 
	                                id="name" name="name" class="form-control" placeholder="姓名" value="${sessionScope.loginUser.name}">
	                            </div>
	                        </div>
	                        ...
	                        <div class="form-group">
	                            ...
	                            <div class="col-sm-10">
	                            <select class="form-control" name="dempartment" id="dempartment">
	                            <!-- depList -->
	                            	 <c:forEach items="${depList}" var="d" varStatus="status">
                      	...
                      	<div class="col-lg-6">
                      		 <div class="form-group">
	                            ...
	                            <div class="col-sm-10">
	                                <input  value="${newDate}" aria-required="true" type="text" 
	                                id="sdate" name="sdate" class="form-control" placeholder="开始日期" >
	                            </div>
                        	</div>
							<div class="form-group">
	                            <label class="col-sm-2 control-label">结期</label>
	                            <div class="col-sm-10">
	                            	<input type="text" value="${newDate}" id="edate" name="edate" class="form-control" placeholder="结束日期">
	                            </div>
	                        </div>
                      ...
                      <div class="col-lg-12">
                      		 <div class="form-group">
                            <label class="col-sm-1 control-label">审请理由</label>
                            <div class="col-sm-11">
                            	<textarea id="message" name="message" data-rangelength="[2,1000]" placeholder="审请理由" class="form-control parsley-validated" rows="3"></textarea>
                            </div>
                        </div>
                        </div>
                        ...
                        <div class="col-lg-12">
                      		 <div class="form-group">
                            <label class="col-sm-1 control-label">准备事宜：</label>
                            <div class="col-sm-11">
                            	<textarea id="repairsAddredd" name="repairsAddredd" data-rangelength="[2,1000]" placeholder="准备事情" class="form-control parsley-validated" rows="3"></textarea>
                            </div>
                        </div>
                        </div>
                      </div>
					...
					<!-- idcode=1 表示是请假 -->
					<input id="idCode" name="idCode" type="hidden" value="4">
                <div class="modal-footer">
                    <button type="button" class="btn btn-white" data-dismiss="modal">关闭</button>
 ...
 <script type="text/javascript">
 	...
 	//onclick="updateFormValue('${item.id}','${item.name}','${item.sdate}','${item.dempartment}',
    //'${item.edate}','${item.type}','${item.message}','${item.repairsAddredd}')">编辑</button>
 	function updateFormValue(id,name,sdate,dempartment,edate,type,message,repairsAddredd){
 		$('#myModal5').modal('show');
 		$("#id").val(id);
		$("#name").val(name);
		$("#dempartment").val(dempartment);
		$("#sdate").val(sdate);
		$("#edate").val(edate);
		$("#type").val(type);
		$("#message").val(message);
		...
		$("#dempartment").find("option[value='"+dempartment+"']").attr("selected", "selected");
		
		//标题设置，先清空再添加
		$("#panelTitle").empty();
		$("#panelTitle").append("更新专用室"); 
 	}
 	//添加时－清空值
 	...
 	function deleteById() {
 		...
 		window.location.href = "delete?idCode=4&id="+deleteId;
 	}
