	           	 ...
	           	 <li class="dropdown">
	           	 	<c:choose>
	           	 		<c:when test="${not empty items.listPermissionSubMenu}">
	           	 			<a  href="${pageContext.request.contextPath}/web/civilization/${year}/list?isMenu=two&isone=yes&superid=${pm.id}&id=${items.id}&moduleId=${items.id}&menuName=${items.menuName}" data-toggle="dropdown" class="dropdown-toggle">${items.menuName}</a>
	           	 			<ul class="dropdown-menu" role="menu">
		                  <c:forEach items="${items.listPermissionSubMenu}" var="subitems" varStatus="status">
		                            <li>
		                            <a style="width:180px;" href="${pageContext.request.contextPath}/web/civilization/${year}/list?isMenu=three&isone=yes&superid=${pm.id}&id=${items.id}&moduleId=${subitems.id}&menuName=${subitems.menuName}">
		                            ${subitems.menuName}
		                            </a></li>
		                  </c:forEach>
	           	 		...
	           	 		<c:otherwise>
	           	 			<a href="${pageContext.request.contextPath}/web/civilization/${year}/list?isone=yes&isMenu=three&superid=${superid}&id=${pm.id}&moduleId=${items.id}&menuName=${items.menuName}" data-toggle="dropdown" class="dropdown-toggle">
	           	 			${items.menuName}</a>
	           	 		</c:otherwise>
	           	 	</c:choose>
	...
	<div class="i_ma clearfix" style="width:200px">
    	<div class="i_mam" style="width:200px">
            ...
            <ul class="i_news" style="width:200px">
            	...
            	<li><a target="_blank" href="${pageContext.request.contextPath}/web/civilization/${year}/find?superid=null&id=null&newsId=${news.id}&menuName=${news.title}">
            			 <c:choose>
			        ...
			        <div class="i_mbr">
			        	<div class="box_h">
							...
							<c:forEach var="news" items="${newslist.datas}">
			            		<li><a target="_blank"  href="${pageContext.request.contextPath}/web/civilization/${year}/find?superid=${superid}&id=${pm.id}&newsId=${news.id}&menuName=${news.title}">
<%-- 			            		<c:if test="${isone=='yes'}"> --%>
<%-- 			            			&lt;${news.moduleName}> --%>
<%-- 			            		</c:if> --%>
						...
						<a class="navlinks"
							href="${pageContext.request.contextPath}/web/civilization/${year}/list?isMenu=${isMenu}&isone=${isone}&pageNo=${newslist.upPage}&superid=${superid}&id=${id}&moduleId=${moduleId}&menuName=${pm.menuName}">&lt;
							上一页</a>
							...
							<c:choose>
								<c:when test="${nav == pageList.pageNo}">
									...
									<a class="navlinks"
										...
										href="${pageContext.request.contextPath}/web/civilization/${year}/list?isMenu=${isMenu}&isone=${isone}&pageNo=${nav}&superid=${superid}&id=${id}&moduleId=${moduleId}&menuName=${pm.menuName}">
										${nav}</a>
								</c:when>
								<c:otherwise>
									<a class="navlinks"
						...
						<a class="navlinks"
							href="${pageContext.request.contextPath}/web/civilization/${year}/list?isMenu=${isMenu}&isone=${isone}&pageNo=${newslist.nextPage}&superid=${superid}&id=${id}&moduleId=${moduleId}&menuName=${pm.menuName}">下一页
							></a>
