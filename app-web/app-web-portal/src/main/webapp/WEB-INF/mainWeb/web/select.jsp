			...
			<div class="main_box_inner">
				...
				<div class="main_box_inner_right">
					<div class="current_location current_location_547">
						<ul>
							<li class="current_location_1">搜索到的内容如下：
							</li>
						</ul>
					</div>
					<div class="sublanmu_box sublanmu_box_547">
						...
						<a href="${pageContext.request.contextPath}/web/select?pageNo=1&name=${name}&menuId=${menuId}" class="extend">第一页</a>
						<a href="${pageContext.request.contextPath}/web/select?pageNo=${newslist.upPage}&name=${name}&menuId=${menuId}">上一页</a>
                        ...
                        <c:choose>
                        	...
                        	<c:otherwise>
	                        	<a href="${pageContext.request.contextPath}/web/select?pageNo=${nav}&name=${name}&menuId=${menuId}">
	                        	${nav}</a>
                        	</c:otherwise>
                        </c:choose>
                        </c:forEach>
                        
						<a href="${pageContext.request.contextPath}/web/select?pageNo=${newslist.nextPage}&name=${name}&menuId=${menuId}">下一页</a>
						<a href="${pageContext.request.contextPath}/web/select?pageNo=${newslist.totalPage}&name=${name}&menuId=${menuId}" class="extend">最后一页</a>
					</c:if>
			</div>
					</div>
				</div>
